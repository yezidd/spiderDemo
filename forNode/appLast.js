...
async function run() {
  ...
  let courseList = await getCourseListAll(array);

  await getVideoUrlList(courseList);

}

async function getVideoUrlList(list) {
  return await new Promise(async (resolve, reject) => {
    console.log(data, "总的数据")
    async.eachSeries(data, async function (item) {
      consoel.log(item, "----1--2-3---")
      let lessonUrl = await getVideoUrl(item.lessonURL);

    });
  });
}

async function getVideoUrl(v) {
  console.log(v, "视频列表这边")
  return new Promise((resolve, reject) => {
    request.get(v.lessonURL)
      .set(base_config)
...
async function getCourseListAll(array) {

  return await new Promise(function (resolve, reject) {

    async.eachSeries(array, async function (item) {
      let lessonUrl = await getCourseList(item);
